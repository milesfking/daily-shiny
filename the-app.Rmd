...
This was pretty straightforward, using a very similar function to the NASA photo of the day. Instead of using an official NBA API, we opted for the "balldontlie" NBA API, which doesn't require a key to authorize requests to the API. The JSON list returned is a list of all games on a certain da, with information including score, who the home team was, and who the away team was. Our goal for this component of the project was to output a nicely formatted list of all of the scores, making it obvious using HTML text colors who the winner and loser of each individual game was. We accomplished this by outputting text using the `fluidRows()` method similar to the UI output that was used to output the example links in the shell of the NYT Shiny app. This was ultimately successful in outputting the scores in an simple, comprehensible, and visually appealing manner.
...
```{r}
Sys.setenv(SPOTIFY_CLIENT_ID = '14f9f9b94733459cb9cdb896d9e2530a')
...
We were succesful in accessing and wrangling data from the NASA API. This API was very simple to register for and code in, using code inspired from what we learned to make the NYT API. Specifically, we looked at the "Astronomy Picture of the Day" data from NASA and access historical images and explanations for a given date using the `get_nasa_photos()` button. The JSON returned by the function is easy to navigate as the list elements are clearly labeled `url` and `explanation`, and we integrated it into the Shiny app in a similar fashion to the NYT links (via an `actionButton`  that results in a modaDialog with further information upon a click).
...
print(get_nasa_photos(2020,8,3, NASA_key))
```
...
shinyApp(
  ...
  server = function(input, output, session) {
    ...
    observeEvent(input$search, {
      ...
      # button in shiny app that allows you to find out more
      output$nasa_link = renderUI({
        h4(actionLink("explanation", paste(nasa$title)))
      })
      
      for(i in seq_along(state$nasa_observers)) {
        state$nasa_observers[[i]]$destroy()
      }
      
      # creates modal dialog with description of the image and link to it
      state$nasa_observers[[1]] = observeEvent(input$explanation, ignoreInit = TRUE, {
        showModal(modalDialog(
          title = paste(nasa$title),
          div(nasa$explanation),
          footer = tagList(
            actionButton(inputId='read', label="See Image",
                         class = "btn btn-primary",
                         icon = icon("th"), 
                         onclick = paste0("window.open('", nasa$url,  "', '_blank')")),
            modalButton("Return")
          )
        ))
      })
    ...
    observeEvent(input$search, {
      ...
      output$scores = renderUI(fluidPage(ui_elems))
